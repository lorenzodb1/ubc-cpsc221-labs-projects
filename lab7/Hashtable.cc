...
void Hashtable::qinsert(int k) {
	...
	for (int i = 0; i < _size; i++) {
		if (_table[(hash(k) + i*i) % _size] == -1) {
			_table[(hash(k) + i*i)%_size] = k;
			tallyProbes(i);
			return;
		}
	}
...
void Hashtable::linsert(int k) {
	...
	for (int i = 0; i < _size; i++) {
		if (_table[(hash(k) + i) % _size] == -1) {
			_table[(hash(k) + i)%_size] = k;
			tallyProbes(i);
			return;
		}
	}
...
void Hashtable::dinsert(int k) {
	...
	r = 1999993;
	for (int i = 0; i < _size; i++) {
		if (_table[(hash(k) + (i^2)*(k%r)) % _size] == -1) {
			_table[(hash(k) + (i^2)*(k%r))%_size] = k;
			tallyProbes(i);
			return;
		}
	}
	...
	/*for (int i = 0; i < _size; i++) {
		if (_table[(hash(k) + i*(k%r)) % _size] == -1) {
			_table[(hash(k) + i*(k%r)) % _size] = k;
			tallyProbes(i);
			return;
		}
	}*/
