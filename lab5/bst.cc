...
int numNodes( Node* root ) {
	...
	if (root->left != NULL) {
		num = num + numNodes(root->left);
	}
	if (root->right != NULL) {
		num = num + numNodes(root->right);
	}
	return num;
}
...
int numLeaves( Node* root ) {
	...
	if (root->left != NULL) {
		num = num + numLeaves(root->left);
	}
	if (root->right != NULL) {
		num = num + numLeaves(root->right);
	}
	return num;
}
...
int height( Node* x ) {
	...
	int left = height(x->left) + 1;
	int right = height(x->right) + 1;
	if (left > right) {
		return left;
	}
	else return right;
...
bool helperLeft(Node* root, Node* curr) {
	if (curr->left != NULL && curr->right == NULL) {
		return helperLeft(root, curr->left) && curr->key <= root->key;
	}
	else if (curr->left == NULL && curr->right != NULL) {
		return helperLeft(root, curr->right) && curr->key <= root->key;
	}
	else if (curr->left != NULL && curr->right != NULL) {
		return helperLeft(root, curr->left) && helperLeft(root, curr->right) && curr->key <= root->key;
	}
	return curr->key <= root->key;
}

bool helperRight(Node* root, Node* curr) {
	if (curr->left != NULL && curr->right == NULL) {
		return helperRight(root, curr->left) && curr->key >= root->key;
	}
	else if (curr->left == NULL && curr->right != NULL) {
		return helperRight(root, curr->right) && curr->key >= root->key;;
	}
	else if (curr->left != NULL && curr->right != NULL) {
		return helperRight(root, curr->left) && helperRight(root, curr->right) && curr->key >= root->key;
	}
	return curr->key >= root->key;
}
...
bool is_bst(Node* root) {
	...
	*/
	if (root->left != NULL && root->right == NULL) {
		return is_bst(root->left) && helperLeft(root, root->left);
	}
	else if (root->left == NULL && root->right != NULL) {
		return is_bst(root->right) && helperRight(root, root->right);
	}
	else if (root->left != NULL && root->right != NULL) {
		return helperLeft(root, root->left) && helperRight(root, root->right) && is_bst(root->left) && is_bst(root->right);
	}
	return true;
}
...
/*bool is_bst(Node* root) {
	 ...
	 *//*
	if (root->left != NULL && root->right == NULL) {
		return is_bst(root->left) && (root->left)->key <= root->key;
	}
	else if (root->left == NULL && root->right != NULL) {
		return is_bst(root->right) && (root->right)->key >= root->key;
	}
	else if (root->left != NULL && root->right != NULL) {
		return is_bst(root->left) && is_bst(root->right) && (root->left)->key <= root->key && (root->right)->key >= root->key;
	}
	return true;
}*/
...
void runMain(Node*& tree, std::vector<int> keys) {
	...
	std::cout << std::endl;
	std::cout << "is_bst() Test:" << std::endl;
	std::cout << std::endl;

	printTree(root);

	std::cout << std::endl;
	if (!is_bst(root)) std::cout << "This tree is not bst" << std::endl;
	else std::cout << "Tree is bst" << std::endl;
	std::cout << std::endl;
}
